...
class AudioDataMerge(
    ...
    suspend fun merge() = withContext(Dispatchers.Default) {
        ...
        suspend fun extractVideoFile(path: String) {
            // 動画の情報を読み出す
            val (_mediaExtractor, index, format) = MediaExtractorTool.extractMedia(path, MediaExtractorTool.ExtractMimeType.EXTRACT_MIME_AUDIO) ?: return
